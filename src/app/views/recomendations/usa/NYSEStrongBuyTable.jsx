...
import { fetchNYSEStrongBuyAnalysis } from "/src/__api__/db/apiService"; // Assure pathways are correct
...
export default function NYSEStrongBuyTable() {
  const [data, setData] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await fetchNYSEStrongBuyAnalysis();
        setData(result.data);
      } catch (error) {
  ...
  return (
    <StyledTableContainer component={Paper}>
      <StyledTable stickyHeader size="small" aria-label="strong buy analysis table">
        <TableHead>
          <TableRow>
